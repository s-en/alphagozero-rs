...
class DualNet(nn.Module):
    ...
    def __init__(self, board_size, action_size, num_channels):
        ...
        self.conv1 = nn.Conv2d(15, num_channels, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        x = x.view(-1, 15, self.board_size, self.board_size)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.layers(x)
...
if __name__ == '__main__':
    ...
    dummy = torch.rand([1, 15, 5, 5]).to(device)
    traced_net = torch.jit.trace(model, dummy)
    # print(traced_net)
    # for param in model.parameters():
    #     print(param)
    traced_net.save("dualnet5x5.pt")
    print("saved model")
